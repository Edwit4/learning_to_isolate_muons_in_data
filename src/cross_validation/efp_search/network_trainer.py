...
if __name__ == '__main__':
    ...
    parser.add_argument('iteration_num', metavar='i', type=int, nargs=1,
        help='EFP search iteration')
    ...
    iteration_num = args.iteration_num[0]
    ...
    version = f'numcones_{num_cones}_trainsize_{train_size}_fold_{kfold_index}_iteration_{iteration_num}'
    experiment_dir = './src/training/efp_search'
    version_dir = os.path.abspath(f'{experiment_dir}/lightning_logs/{version}')
    if iteration_num > 0:
        prev_version = f'iteration_{iteration_num-1}'
        prev_version_dir = os.path.abspath(
            f'{experiment_dir}/lightning_logs/{prev_version}')
    ...
    if iteration_num > 0:
        with open(f'{prev_version_dir}/kappa_betas.pickle','rb') as handle:
            selected_kappa_betas = pickle.load(handle)
        with open(f'{prev_version_dir}/graphs.pickle','rb') as handle:
            selected_graphs = pickle.load(handle)
