...
if __name__ == '__main__':
    ...
    version = f'trainsize_{train_size}_bootstrap_{bootstrap_index}'
    data_dir = os.path.abspath('./src/data')
    experiment_dir = os.path.abspath('./src/bootstrapping/pfn')
    ...
    data = pfn_lmdb_data(data_dir, batch_size=int(effective_batch_size/num_gpus),
        num_folds=num_folds, kfold_index=kfold_index, test_size=test_size,
        train_size=train_size, pT_bound=pT_bound, eta_bound=eta_bound, M_lo=M_lo,
        M_hi=M_hi, pos_lo=pos_lo, pos_hi=pos_hi, scaler=scaler, 
        rng=np.random.default_rng(seed=seed), bootstrap_seed=bootstrap_index+seed)
    model = pfn(Phi_size, F_size, l_size, effective_learning_rate*num_gpus, 
        version_dir, patience=patience, min_change=min_change)
